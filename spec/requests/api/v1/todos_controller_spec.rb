...
RSpec.describe Api::V1::TodosController, type: :controller do
  ...
  it '#index responds a 401 response (not autorized)' do
    response = HTTParty.get("#{url_remote}/api/v1/todos", headers: headers)
  ...
  describe 'as Valid Token' do
    ...
    it '#index include valid response' do
      # user = User.first
      response = HTTParty.get("#{url_remote}/api/v1/todos", headers: headers_auth)
    ...
    it '#create if return valid token' do
      response = HTTParty.post("#{url_remote}/api/v1/todos", { body: {description: "Task test"}.to_json, headers: headers_auth })
      expect(response.code).to eq(200)
    ...
    it '#destroy with authentication' do
      response = HTTParty.delete("#{url_remote}/api/v1/todos/#{task_id}", headers: headers_auth)
      expect(response.code).to eq(200)
      todo_response = JSON.parse(response.body)
    ...
    it '#update with authentication' do
      response = HTTParty.patch("#{url_remote}/api/v1/todos/#{task_id}", { body: {description: "Task MODIFIED"}.to_json, headers: headers_auth })
      expect(response.code).to eq(200)
      todo_response = JSON.parse(response.body)
    ...
    it '#done_task with authentication' do
      response = HTTParty.post("#{url_remote}/api/v1/todos/#{task_id}/done_task", { headers: headers_auth })
      expect(response.code).to eq(200)
      todo_response = JSON.parse(response.body)
